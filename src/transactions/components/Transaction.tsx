...
const Transaction = ({ id }: TransactionProps) => {
	const [transaction, setTransaction] = useState<any>(),
		[processorDetails, setProcessorDetails] = useState<any>(),
	...
	useEffect(() => {
		...
		apiClient
			.get(`/transactions/${id}/processor-details`)
			.then(setProcessorDetails)
			.catch(e => console.error('Transaction API Error', e));

		apiClient
			.get(`/transactions/${id}/tickets`)
			.then(setTickets)
			.catch(e => console.error('Transaction API Error', e));
	...
	const refund = useCallback(() => {
		...
		apiClient
			...
			.then(() => Promise.all([apiClient.get(`/transactions/${id}`).then(setTransaction), apiClient.get(`/transactions/${id}/processor-details`).then(setProcessorDetails)]))
			.catch(e => console.error('Transaction API Error', e))
	...
	const transfer = useCallback(() => {
		...
		apiClient
			...
			.then(() => Promise.all([apiClient.get(`/transactions/${id}`).then(setTransaction)]))
			.catch(e => console.error('Transaction API Error', e))
	...
	const { firstName, lastName, email, amount, braintreeTransactionId, originalTransactionId, type: transactionType, transfereeId, created, order, status: transactionStatus } = transaction,
		{ status: processorStatus, createdAt, refundId, creditCard, applePay, paymentInstrumentType, merchantId } = processorDetails || {};

	const transactionDate = new Date(createdAt),
		refundCutoff = Date.now() - 90 * 24 * 60 * 60 * 1000, // 90 days
		refundAllowed = transactionDate.getTime() > refundCutoff;
		...
		<div className={styles.transaction}>
			{transactionType !== 'transfer' ? (
				<h1>
					{}
					Transaction -{' '}
					<a href={`https://www.braintreegateway.com/merchants/${merchantId}/transactions/${braintreeTransactionId}`} target="_blank" title="Open in Braintree">
				...
				<h1>
					{}
					Transferred from Transaction -{' '}
					<Link to={`/transactions/${originalTransactionId}`} target="_blank" title="Open Original Transaction">
			...
			<div className={styles.flexRow}>
				<div className={styles.flexItem}>
					...
					{paymentInstrumentType === 'credit_card' && (
						<h3>
							<span>Card Used:</span> {creditCard.cardType} (...{creditCard.last4}) - exp. {creditCard.expirationDate}
						</h3>
					)}
					{paymentInstrumentType === 'apple_pay_card' && (
						<h3>
							<span>Apple Pay Used:</span> {applePay.paymentInstrumentName}
						</h3>
					)}
					{transactionType !== 'transfer' && (
						<h3>
							<span>Purchased:</span> {format(new Date(created), 'M/dd/yy - HH:mm')}
						</h3>
					)}
					{transactionType === 'transfer' && (
						<h3>
							<span>Transferred:</span> {format(new Date(created), 'M/dd/yy - HH:mm')}
						</h3>
					)}
					<h3>
					...
					{transactionType !== 'transfer' && (
						<h3 className={classnames('processor-status', processorStatus)}>
							<span>Processor Status:</span> {processorStatus}
						</h3>
					)}
					{}
					{refundId && (
						<h3>
							...
							<a href={`https://www.braintreegateway.com/merchants/${merchantId}/transactions/${refundId}`} target="_blank" title="Open in Braintree">
								{refundId}
							</a>
						</h3>
					)}
					...
					{transactionType !== 'transfer' && (
						<>
							...
							<ul className={styles.orderDetails}>
								{order.map(({ productId, quantity }: any) => {
									const { name, price, eventId } = products.find((p: any) => p.id === productId) || {};
									...
									return (
										<li key={productId}>
											...
											{eventId && <p className="event">{events.find((e: any) => e.id === eventId)?.name}</p>}
										</li>
									);
								})}
							</ul>
				...
				<div className={styles.flexItem}>
					...
					{transactionToken && (
						<p>
							<a href={`https://mustachebash.com/my-tickets?t=${transactionToken}`} target="_blank">
								Ticket Link
							</a>
						</p>
					)}
					{!!tickets.length && <TicketsList tickets={tickets} selectedTickets={[]} setSelectedTickets={() => {}} />}
					...
					<div>
						...
						{!['refunded', 'voided', 'transferred'].includes(transactionStatus) && (
							<>
								...
								<button
									className="red"
									onClick={transfer}
									disabled={transferring || !(transfereeFirstName && transfereeLastName && transfereeEmail && /.+@.+\..+/.test(transfereeEmail))}
								>
						...
						{transactionStatus === 'transferred' && (
							<h5>
								<Link to={`/transactions/${transfereeId}`} target="_blank" title="Open Original Transaction">
									View Transfer - {transfereeId.substring(0, 8)}{' '}
								</Link>
							</h5>
						)}
					</div>
